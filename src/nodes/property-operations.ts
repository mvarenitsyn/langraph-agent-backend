...
`;
    ...
    console.log('[PropertyOperations] ✓ Tool execution completed');
    ...
    // If so, skip response generation - response-synthesizer will create the final response
    const isMultiStepTask = state.taskList && state.taskList.tasks.length > 1;
    if (isMultiStepTask) {
      console.log('[PropertyOperations] Multi-step task detected - skipping response generation');
      console.log('[PropertyOperations] Response will be generated by response-synthesizer node');
      return {
        toolResults,
        platformContext,
      };
    ...
    // Generate final response for single-task workflows
    console.log('[PropertyOperations] Single task - generating final response...');
...
- Highlight key features for property details
